...
const OUT = "/Users/justus/obsidian_atomized_intermediary/lab/results/v6-channels.json";
...
const CH = { RA: chRA, katz: chKatz, ppr: chPPR, heat: chHeat, simrank: chSimRank,
...
for (let s = 0; s < SEEDS; s++) {
  ...
  const base = evaluate((i, a) => znorm(CH.RA(i, a)), test, adj).recall;
  const baseReal = evaluate((i, a) => znorm(CH.RA(i, a)), testReal, adj).recall;
  for (const n of NAMES) {
    ...
    if (n === "RA") continue;
    const fus = (i, a) => { const o = new Map(znorm(CH.RA(i, a)));
...
console.log("\n  MARGINAL on top of RA    ALL              REAL-only          verdict (2 SD)");
...
writeFileSync(OUT, JSON.stringify({ notes: N, edges: edges.length, realEdges: nReal, seeds: SEEDS,
  all, real, marginal: marg, marginalReal: margReal, fires, cond }, null, 1));
console.log("\nwrote", OUT);
