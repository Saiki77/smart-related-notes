...
> | 4.0.0-beta.3 (2026-09-11) | **offline setup automated + tucked away**: guided modal (command palette, or one button on a failed-download error; the settings pane itself stays clean) — "Open downloads in browser" opens every missing file, then a 3 s watcher on the Downloads folder imports + verifies each file as it lands (poll-idempotent, partials ignored, one import at a time); manual picker only as fallback. Folder-scan path validated end-to-end in Node |
